...
class Trainer:
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if logits is not None:
                ...
                logits = self._nested_gather_object(logits)
                preds_host = logits if preds_host is None else preds_host + logits
            if labels is not None:
            ...
            if self.args.eval_accumulation_steps is not None and (step + 1) % self.args.eval_accumulation_steps == 0:
                ...
                if preds_host is not None:
                    all_preds = preds_host if all_preds is None else all_preds + preds_host
    ...
    def prediction_step(
        ...
        logits = nested_detach(logits) # logits, hidden state, attention
        # if len(logits) == 1:
        # logits = logits[0].cpu().numpy() # (B, C)
        logits = logits[0]

        # select_ids = np.argmax(logits, axis=1) # (B,)
        select_ids = torch.argmax(logits, dim = 1) # (B,)

        targets = []
        preds = []
        for i,d in enumerate(sample_data):
            targets.append(d['target'])
            preds.append(d['candidates'][select_ids[i]])
