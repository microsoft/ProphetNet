...
@dataclass
class SampleEncoderDecoderOutput_ours(ModelOutput):
    """
    ...
    seq_lens: torch.LongTensor = None
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def generate_for_training(
        ...
        **model_kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            # sample
            return self.sample_for_training(
                input_ids,
                logits_processor=logits_processor,
    ...
    def sample_for_training(
        ...
        synced_gpus: Optional[bool] = None,
        **model_kwargs,
    ) -> Union[SampleEncoderDecoderOutput_ours, torch.LongTensor]:
        r"""
        ...
        scores = ()
        seq_lens = torch.zeros((input_ids.size(0)), dtype = torch.long, device=input_ids.device)
        decoder_attentions = () 
        cross_attentions = () 
        ...
        while True:
            ...
            next_tokens = torch.multinomial(probs, num_samples=1).squeeze(1) # (B,) each token
            ...
            seq_lens += unfinished_sequences # to calculate the sequence lengths
        ...
        scores = torch.stack(scores) #(L, B, V)
        scores = scores.transpose(0,1)

        if return_dict_in_generate:
            return SampleEncoderDecoderOutput_ours(
                sequences=input_ids,
                scores=scores,
                seq_lens=seq_lens,
            )
